...
//tipo --> SS:Sust   AA:Adj   AI:Adj inmóvil	VV: Verbo
  ...
  DictRed* dictWrds = [DictRed GetForLang:hData->m_Idioma];
	...
	if( tipo == "VV" || tipo == "VA" || tipo == "BE" || tipo == "HA")
		...
		if( VIDic->GetKey(hData->m_Palabra.c_str(), szRoot))
			{
			root = szRoot;
      int typs = [dictWrds FindWord:root.c_str()];
			if( typs!=-1 )
				{
				if( [dictWrds IsTypeVerb:typs] )
					reducida = VerbRedCheck(hData,root,"VV");
				else
					{
					if( [dictWrds IsTypeVerbAux:typs]  )
						reducida = VerbRedCheck(hData,root,"VA");
					else
						if( [dictWrds IsTypeBeOrHa:typs] )
							reducida = VerbRedCheck(hData,root,"BE");
      ...
      int typs = [dictWrds FindWord:root.c_str()];
			if( typs!=-1 )
				...
				if( tipo == "SS")
					{
					if( [dictWrds IsTypeSustantive:typs] )
						reducida = NounRedCheck(hData,root);
					}
				else
					{
					if( ([dictWrds IsTypeAdjetive:typs] || [dictWrds IsTypeAdjetiveInmovil:typs] ) && [dictWrds IsTypeProname:typs] )
						reducida = AdjRedCheck(hData,root,"AO");
					else
						{
						if( [dictWrds IsTypeAdjetive:typs] )
							reducida = AdjRedCheck(hData,root,"AA");
						else
							{
							if( [dictWrds IsTypeAdjetiveInmovil:typs] )
								reducida = AdjRedCheck(hData,root,"AI");
							else
								if( [dictWrds IsTypeProname:typs] )
									reducida = AdjRedCheck(hData,root,"OO");
								else
									if( [dictWrds IsTypeArticle:typs] )
										reducida = AdjRedCheck(hData,root,"RD");
							}
						}
					}
				}
...
//tipo --> 0:Sust y Adj   1: Verbos
  ...
  DictRed* dictWrds = [DictRed GetForLang: hData->m_Idioma];
	...
	for(int j=0; j<2; j++ )
        ...
        int typs = [dictWrds FindWord:raiz.c_str()];
        if( typs != -1 )
					{
					//Si tipo es sustantivo
					if( tipo == "SS")
						...
						if( tipo == "VV" || tipo == "VA" || tipo == "BE" || tipo == "HA")
							...
							if( [dictWrds IsTypeVerb:typs] )
								...
								if( [dictWrds IsTypeVerbAux:typs] )
									...
									if( [dictWrds IsTypeBeOrHa:typs] )
										reducida = VerbRedCheck(hData,raiz,"BE");
								}
							}
						else
							{
							if( ([dictWrds IsTypeAdjetive:typs] || [dictWrds IsTypeAdjetiveInmovil:typs]) && [dictWrds IsTypeProname:typs] )
								...
								if( [dictWrds IsTypeAdjetive:typs] )
									...
									if( [dictWrds IsTypeAdjetiveInmovil:typs] )
										...
										if( [dictWrds IsTypeProname:typs] )
											reducida = AdjRedCheck(hData,raiz,"OO");
										else
											if( [dictWrds IsTypeArticle:typs] )
												reducida = AdjRedCheck(hData,raiz,"RD");
									}
								}
							}
						}
...
BOOL CRedWord::ReduceSimple( CRWhData* hData)
  ...
  int typs = [dictWrds FindWord:hData->m_Palabra.c_str()];
  if( typs != -1 )
		{
		//Si tipo es sustantivo
		if( [dictWrds IsTypeSustantive:typs] )
		...
		if( [dictWrds IsTypeAdjetive:typs] || [dictWrds IsTypeAdjetiveInmovil:typs] ||
        [dictWrds IsTypeProname:typs]  || [dictWrds IsTypeArticle:typs] )
			{
			reducida = TRUE;
		...
		if( [dictWrds IsTypeVerb:typs] || [dictWrds IsTypeVerbAux:typs] || [dictWrds IsTypeBeOrHa:typs] )
			{
			reducida = TRUE;
			if( [dictWrds IsTypeVerb:typs] )
				tipo = "VV";
		...
		//Si tipo es adverbio
		if( [dictWrds IsTypeAdvervio:typs] )
			{
			reducida = TRUE;
			CRWType redtype;
